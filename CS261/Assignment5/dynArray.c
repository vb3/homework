...
void addHeap(DynArr *heap, TYPE val)
{
    ...
    while(compare(heap->data[pos], heap->data[(pos-1)/2]) == -1){
        swapDynArr(heap, pos, (pos-1)/2);
        pos = (pos-1)/2;
    }
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
    ...
    if(rightChild < max){ //two children
        ...
        if(compare(heap->data[pos], heap->data[smallChild]) == 1){ //if value at pos > value of smallest child
            swapDynArr(heap, pos, smallChild);
            _adjustHeap(heap, max, smallChild);
        }
    }
    else if(leftChild < max) {
        if(compare(heap->data[pos], heap->data[leftChild]) == 1){ //if value at pos > value of child
